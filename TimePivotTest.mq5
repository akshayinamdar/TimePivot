...
// This file is designed specifically for testing TimePivot EA in the Strategy Tester
// It contains the same logic but with additional testing metrics and optimizations
...
int OnInit()
{
   ...
   // Cannot use ArrayInitialize with MqlTick structure
   ...
   ArrayInitialize(TickRanges, 0.0);
   ...
   Print("TimePivot Test initialized successfully");
...
{
   // Print test results
   Print("------ TimePivot Test Results ------");
   Print("Total Trades: ", TotalTrades);
   Print("Winning Trades: ", WinningTrades, " (", (TotalTrades > 0 ? (WinningTrades*100.0/TotalTrades) : 0), "%)");
   Print("Losing Trades: ", LosingTrades, " (", (TotalTrades > 0 ? (LosingTrades*100.0/TotalTrades) : 0), "%)");
   Print("Total Profit: ", TotalProfit);
   Print("Total Loss: ", TotalLoss);
   Print("Net Profit: ", TotalProfit + TotalLoss);
   Print("Maximum Drawdown: ", MaxDrawdown);
   Print("Profit Factor: ", (TotalLoss != 0 ? MathAbs(TotalProfit/TotalLoss) : 0));
   Print("----------------------------------");
   ...
   Print("TimePivot Test deinitialized, reason: ", reason);
}
         ...
         for(int j = 0; j < HistoryDealsTotal(); j++) {
            ...
            if(HistoryDealGetInteger(dealTicket, DEAL_POSITION_ID) == OpenTrades[i].ticket) {
               ...
               if(dealProfit > 0) {
                  ...
                  LosingTrades++;
                  TotalLoss += dealProfit; // TotalLoss will be negative
                  ConsecutiveLosses++;
                  ConsecutiveWins = 0;
               }
